...
Ndata = 2   # Paired conditions
#reading_folder  = ['Data_Control', 'Data_MotorThalamusInactivation']
reading_folder  = ['Data_Control', 'Data_NoradrenalineBlock']
#pick_label_pre = ['v56_batch23_0_0_', 'v56_batch20_0_1_0_']
pick_label_pre = ['v56_batch23_0_0_', 'v56_batch22_']
...
for Tw in Twindow:
    ...
    for n_components in [3]: #[2,3]:    
        ...
        if n_components==3 and Tw==25:
            ...
            # Plot data
            #color1 = [ (0.175, 0.35, 0.66, 1), (0.47, 0.81, 0.92, 1)]          
            color1 = [ (0.175, 0.35, 0.66, 1), (0.55, 0.76, 0.36, 1)]
            #color2 = [ (0.90, 0.21, 0.16, 1), (0.61, 0.34, 0.67, 1)]
            color2 = [ (0.90, 0.21, 0.16, 1), (0.95, 0.70, 0.36, 1)]
            ...
            for az in np.linspace(0,360,37):
                ...
                for i,lab in enumerate(pick_label_pre):
                    ...
                    if i == 0: # control
                        ax.scatter3D(umap_data[label][0:index_on,0], umap_data[label][0:index_on,1], umap_data[label][0:index_on,2], color=color1[0], s=25, alpha = 1.0)
                        ax.scatter3D(umap_data[label][index_on:index_end,0], umap_data[label][index_on:index_end,1], umap_data[label][index_on:index_end,2], color=color2[0], s=25, alpha = 1.0)

                        ax.plot3D(data_i[0:int(len(data_i)/2),0], data_i[0:int(len(data_i)/2),1], data_i[0:int(len(data_i)/2),2], color=color1[0], linewidth=1.0)
                        ax.plot3D(data_i[int(len(data_i)/2):len(data_i),0], data_i[int(len(data_i)/2):len(data_i),1], data_i[int(len(data_i)/2):len(data_i),2], color=color2[0], linewidth=1.0)

                        startpoint1 = ax.scatter3D(umap_data[label][0,0], umap_data[label][0,1], umap_data[label][0,2], color=color1[0], s=150, alpha = 1.0, label='Start (Control)')
                        transitionpoint1 = ax.scatter3D(umap_data[label][index_on,0], umap_data[label][index_on,1], umap_data[label][index_on,2], color=color2[0], edgecolor = 'k', linewidth=2, s=150, alpha = 1.0, label='Transition (Control)')
                        endpoint1 = ax.scatter3D(umap_data[label][index_end-1,0], umap_data[label][index_end-1,1], umap_data[label][index_end-1,2], color=color2[0], s=150, alpha = 1.0, label='End (Control)')

                    else: # mth inact or na-block
                        ax.scatter3D(umap_data[label][0:index_on,0], umap_data[label][0:index_on,1], umap_data[label][0:index_on,2], color=color1[1], s=25, alpha = 1.0)
                        ax.scatter3D(umap_data[label][index_on:index_end,0], umap_data[label][index_on:index_end,1], umap_data[label][index_on:index_end,2], color=color2[1], s=25, alpha = 1.0)

                        ax.plot3D(data_i[0:int(len(data_i)/2),0], data_i[0:int(len(data_i)/2),1], data_i[0:int(len(data_i)/2),2], color=color1[1], linewidth=1.0)
                        ax.plot3D(data_i[int(len(data_i)/2):len(data_i),0], data_i[int(len(data_i)/2):len(data_i),1], data_i[int(len(data_i)/2):len(data_i),2], color=color2[1], linewidth=1.0)

                        startpoint2 = ax.scatter3D(umap_data[label][0,0], umap_data[label][0,1], umap_data[label][0,2], color=color1[1], s=150, alpha = 1.0, label='Start (MTh inact.)')
                        transitionpoint2 = ax.scatter3D(umap_data[label][index_on,0], umap_data[label][index_on,1], umap_data[label][index_on,2], color=color2[1], edgecolor = 'k', linewidth=2, s=150, alpha = 1.0, label='Transition (MTh inact.)')
                        endpoint2 = ax.scatter3D(umap_data[label][index_end-1,0], umap_data[label][index_end-1,1], umap_data[label][index_end-1,2], color=color2[1], s=150, alpha = 1.0, label='End (MTh inact.)')
